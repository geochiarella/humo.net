...
namespace HumoServices.Persistencia
{
    public class BaseDAO <Entidad, Id, Nombre>
    {
        ...
        public int Encontrar(string nombre, Id id)
        {
            using (ISession sesion = NHibernateHelper.ObtenerSesion())
            {
                ICriteria criteria = sesion.CreateCriteria(typeof(Entidad));
                criteria.Add(Restrictions.Eq("Marca", nombre));
                criteria.Add(Restrictions.Eq("Tipo.Codigo", id));
